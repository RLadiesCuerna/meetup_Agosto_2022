...
#Por ejemplo: Filtremos a todos los mamíferos carnívoros que duerman mas de 10 horas
...
# Ejemplo: Ordenamos en función del peso del cerebro:
  
arrange(tmsleep, brainwt)

################
#Elegir las variables por sus nombres (select())

# Ejemplo: Seleccionemos del nombre a la conservación:
...
#Ejemplo: Generemos una nueva tabla con el nombre, todas las columnas que inicen con sleep, y la columna awake
...
# Ejemplo: Esto cambia la unidad de análisis del grupo completo de datos a grupos individuales.

vore<-group_by(msleep,vore)
summarize(vore,mean(sleep_total),mean(sleep_rem,na.rm=TRUE))

#como dijimos antes, dplyr, como el resto de los paquetes de tidyverse, puede ser vista como una gramática de manipulación de datos, donde cada función es un verbo y cada verbo trabaja de manera similar:
  
#  El primer argumento es el nombre del data.frame o tibbies

# Los siguiente argumentos describen que se hace con el data.frame, o tibble o tabla usando los nombres de variables.
